...
from app.database.postgres.models import Chat, ChatParticipant, ChatStatus, ChatType
...
async def handle_match_created(event: Dict[str, Any]) -> bool:
    """
    Обработка события создания мэтча от matching-service.
    Создаёт личный чат между двумя пользователями и сохраняет ID мэтча.
    """
    try:
...
async def register(consumer):
    """Регистрация consumer для событий мэтчей"""
    try:
