...
Поток регистрации
Валидация данных — проверка email и username на уникальность
Создание в Keycloak — создание пользователя с ролью user
Создание в auth-db — сохранение связи keycloak_id ↔ user_id
Создание в user-service — синхронный вызов для создания профиля пользователя
Отправка кода верификации — генерация и отправка 6-значного кода на email
...
user-service публикует USER_PROFILE_UPDATE_REQUESTED
auth-service получает событие и выполняет шаги:
Обновление пользователя в Keycloak
Обновление пользователя в auth-db
Публикация USER_PROFILE_UPDATED (подтверждение)
В случае ошибки выполняется компенсация (откат)
