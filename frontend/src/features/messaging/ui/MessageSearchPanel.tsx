...
export function MessageSearchPanel({ chats, keycloakId, onSelectChat, onClose }: MessageSearchPanelProps) {
      ...
      <div className='relative flex max-h-[70vh] w-full max-w-lg flex-col rounded-2xl bg-white shadow-xl'>
        ...
        <div className='flex-1 overflow-y-auto p-2'>
          {loading ? (
            ...
            <p className='text-secondary p-4 text-center text-[13px]'>Начните вводить текст для поиска</p>
          ) : results.length === 0 ? (
            ...
            results.map((msg) => (
              ...
              >
                <div className='flex items-center justify-between gap-2'>
                  <span className='text-primary text-[13px] font-medium'>{chatNameFor(msg.chat_id)}</span>
                  <span className='text-muted shrink-0 text-[11px]'>{formatDate(msg.created_at)}</span>
                </div>
